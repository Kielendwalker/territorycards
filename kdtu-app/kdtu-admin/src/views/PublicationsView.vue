<template>
  <main class="admin-page">
    <header class="page-head">
      <div>
        <h1>Katalog Terbitan</h1>
        <p class="muted">
          Master data terbitan (Majalah, Risalah, Buku, Brosur, Lainnya). Stok
          di sini adalah stok fisik di gudang; jumlah yang dipasang per
          periode diatur di <router-link to="/timetable">Jadwal</router-link>.
        </p>
      </div>
    </header>
    ...
    <section class="toolbar">
      <label class="search">
        <span>Cari</span>
        <input v-model="search" placeholder="judul / edisi…" />
      </label>
      <label class="filter">
        <span>Kategori</span>
      ...
      <button class="primary" type="button" @click="openCreate">+ Tambah Terbitan</button>
    ...
    <table v-if="rows.length > 0" class="data-table">
      ...
      <tbody>
        <tr v-for="row in rows" :key="row.id">
          <td><span class="badge" :class="`kind-${categorySlug(row.category)}`">{{ row.category }}</span></td>
          <td class="title-cell">{{ row.title }}</td>
          <td class="muted-cell">{{ row.edition || '—' }}</td>
          <td class="num">{{ row.stock }}</td>
          <td class="actions-col">
            <button class="ghost" type="button" @click="openEdit(row)">Edit</button>
            <button class="danger" type="button" @click="askDelete(row)">Hapus</button>
          </td>
        </tr>
      </tbody>
    </table>
    <p v-else-if="!loading" class="empty">
      Belum ada terbitan dalam katalog.
    </p>

    <AdminModal :open="form.open" :title="form.editing ? 'Edit Terbitan' : 'Tambah Terbitan'"
      ...
      <FormField label="Edisi (opsional)" :error="form.errors.edition">
        <template #default="{ id }">
          <input :id="id" v-model="form.edition" :disabled="form.saving" maxlength="200" />
        </template>
      </FormField>
      <FormField label="Stok" :error="form.errors.stock">
        <template #default="{ id }">
          <input :id="id" v-model.number="form.stock" type="number" min="0" :disabled="form.saving" />
        </template>
      </FormField>
...
function emptyForm () {
  return { open: false, editing: null, category: '', title: '', edition: '', stock: 0, errors: {}, serverError: '', saving: false }
}
...
async function reload () {
  ...
  try {
    ...
    const url = '/api/publications' + (params.toString() ? `?${params}` : '')
    rows.value = await api.get(url)
  } catch (err) {
    error.value = `Gagal memuat katalog: ${err.message}`
  } finally {
    loading.value = false
  }
}
...
watch([search, categoryFilter], () => {
  // Debounce-less is fine for in-memory filter; the server is local.
  reload()
})
...
async function submit (close) {
  ...
  try {
    ...
    if (form.editing) {
      await api.put(`/api/publications/${form.editing.id}`, payload)
    } else {
...
function askDelete (row) {
  confirm.title = 'Hapus terbitan?'
  confirm.message = `Hapus "${row.title}" dari katalog? Jika terbitan ini sudah dipasang di satu atau lebih periode, server akan mengembalikan satu lagi atau gagal.`
  confirm.handler = async () => {
    try {
      await api.delete(`/api/publications/${row.id}`)
      await reload()
    } catch (err) {
...
.search, .filter { display: grid; gap: 0.3rem; min-width: 200px; }
.search span, .filter span { font-size: 0.78rem; color: #475569; }
.search input, .filter select {
  padding: 0.4rem 0.5rem; border: 1px solid #cbd5e1; border-radius: 6px; background: white; font: inherit;
}
...
.title-cell { max-width: 40ch; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
